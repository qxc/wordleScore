...
def histToCsv(hist):
    ...
    for key in hist:
        line = key
        #Handles table of values
        if type(hist[key]) == float:        
            line = concatCommaStr(line, hist[key])
        else:
            #Handles table of table of values
            for freq in hist[key]:
                line = concatCommaStr(line, freq)
        strHist = strHist + '\n' + line
    return strHist

#helper function to add a string and a comma to reduce duplicate code
def concatCommaStr(orig, num):
    return orig + "," + str(num)
...
def makeHistogram():
    ...
    for word in WORD_LIST:
        letterPos = 1
        for letter in word:
            if letter in letterHistogram:
                curLetter = letterHistogram[letter]
